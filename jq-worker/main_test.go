...
func TestWorkerRegisterWithOptions(t *testing.T) {
	t.Helper()
	expectedWorkerName := "worker-1003"
	expectedWorkerProcesses := 41
	client := internal.TestSetup(t, internal.WithProcesses(expectedWorkerProcesses), internal.WithHostname(expectedWorkerName))
	defer client.Worker.Close()
	...
	if decWorkerInfo.Name != expectedWorkerName {
		t.Fatalf("expected worker name to be %s, got %s", expectedWorkerInfo.Name, decWorkerInfo.Name)
	}
	if decWorkerInfo.Processes != expectedWorkerProcesses {
		t.Fatalf("expected worker processes to be %d, got %d", expectedWorkerInfo.Processes, decWorkerInfo.Processes)
	}
